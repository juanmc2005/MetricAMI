...
class AMILSTM(nn.Module):
    ...
    def __init__(self, nfeat_word: int, nfeat_sent: int, word_list: list, vec_vocab, dropout: float = 0):
        ...
        self.word_embedding = nn.Embedding(len(word_list), nfeat_word).to(constants.DEVICE)
    ...
    def forward(self, sents: list) -> torch.Tensor:
        ...
        for sent in sents:
            word_ids.append([self.word2id[word]
                             ...
                             for word in sent])
        word_ids = torch.LongTensor(word_ids).to(constants.DEVICE)
        # Get word embeddings
...
class AMIBert(nn.Module):
    ...
    def forward(self, tokens_with_attention_mask: tuple) -> torch.Tensor:
        ...
        # To tensors
        token_ids = torch.tensor(token_ids).to(constants.DEVICE)
        attn_mask = torch.tensor(attn_mask).to(constants.DEVICE)
